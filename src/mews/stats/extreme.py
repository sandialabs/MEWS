...
class DiscreteMarkov:
    ...
    def __init__(
        ...
        if not decay_func_type is None:
            for wt in self._events:
                if (
                    not decay_func_type[wt] is None
                    and not decay_func_type[wt] in self._func_types
                ):
                    raise ValueError(
                        "The decay_func_type['{0}'] input must be a string equal to one of the following: \n\n".format(
                            wt
                        )
                        + str(self._func_types.keys())
                    )
                if not decay_func_type[wt] is None and coef[wt] is None:
                    raise ValueError(
                        "If a 'decay_func_type' is provided, then coefficients input 'coef' is needed."
                    )
        ...
        if state_names is None:
            try:
                ...
                raise Exception(
                    "Something is wrong with the transition_matrix"
    ...
    def history(
        ...
        if isinstance(state0, str):
            if not state0 in self._names:
                raise ValueError(
                    "The 'state0' input must be a valid state_name={}".format(
                        str(self._names)
                    )
                )
            else:
            ...
            raise TypeError(
                "The 'state0' input must be a valid string name or an integer!"
            )
        else:
            nxn = len(self._names)
            if state0 > len(self._names):
                raise ValueError(
                    "The transition_matrix is {0:d}x{1:d} ".format(nxn, nxn)
        ...
        if num_real_step < 0:
            raise ValueError(
                "The number of skip_steps must not be greater than the num_steps!"
            )
        elif num_real_step == 0:
            raise ValueError(
                "There are no real steps. The weather history"
        ...
        if self.use_cython:
            if self.decay_func_type is None or (
                ...
                state = markov_chain_time_dependent_wrapper(
                    cdf,
                    prob,
                    state0,
                    self.coef,
            ...
            if self.decay_func_type is None:
                ...
                state = markov_chain_time_dependent_py(
                    cdf,
                    prob,
                    state0,
                    self.coef,
        ...
        if count_in_min_steps_intervals:
            ...
            for idx in range(big_step):
                ...
                state_count = np.array(
                    [
                        (day == state_val).sum()
                        for name, state_val in self._names.items()
                    ]
                )
                ...
                if prev_state != 0 and new_state != 0 and state_count[0] > 0:
                    ...
                    nstate[min_steps * idx : min_steps * (idx + 1)] = (
                        state_count[1:].argmax() + 1
                    )
                else:
                    nstate[min_steps * idx : min_steps * (idx + 1)] = (
                        state_count.argmax()
    ...
    def steady(self):
        ...
        # matrix
        if (
            self.decay_func_type is None
            or self.decay_func_type == DEFAULT_NONE_DECAY_FUNC
        ):
            ...
            raise NotImplementedError(
                "The 'steady' method does not handle "
    ...
    def _type_checks(self, transition_matrix, state_names):
        ...
        if not isinstance(transition_matrix, np.ndarray):
            ...
            if not np.issubdtype(transition_matrix.dtype, np.number):
                raise TypeError(
                    "The 'transition_matrix' must have elements that are numeric!"
                )
    ...
    def _value_checks(self, state_names, transition_matrix):
        ...
        if (transition_matrix < 0.0).any():
            raise ValueError(
                "All entries to the transition matrix are "
        ...
        if len(state_names) != transition_matrix.shape[0]:
            raise ValueError(
                "The number of statenames must match the size of the transition_matrix!"
            )
        ...
        if (row_sum > 1.0 + epsilon).any() or (row_sum < 1.0 - epsilon).any():
            raise ValueError(
                "The rows of the transition matrix must sum to "
...
class Extremes:
    ...
    def __init__(
        ...
        self._delTmax_verification_data = {
            "delTmax": [],
            "freq_s": [],
        ...
        new_input_format = self._monthly_input_checking(
            [
                max_avg_dist,
                max_avg_delta,
                min_avg_dist,
        ...
        if run_parallel and doe2_input is None:
            ...
            run_parallel = False
            warnings.warn(
                "The program does not handle DOE2 files with"
                ...
                for idy, wfile in enumerate(weather_files):
                    ...
                    if run_parallel:
                        if random_seed is None:
                            ...
                            rng = default_rng(
                                seed=(random_seed + int(1010 * self.rng.random(1)[0]))
                            )
                        args = (
                            start_year,
                            idx,
                        ...
                        results[key_name] = pool.apply_async(
                            self._process_wfile, args=args
                        )
                    else:
                        (results[key_name], objDM[key_name]) = self._process_wfile(
                            start_year,
                            idx,
                            num_wfile,
                            idy,
    ...
    def _monthly_input_checking(self, dict_list):
            ...
            if not isinstance(dict_e, dict) and not dict_e is None:
                ...
                if not dict_e is None and len(dict_e) > 0:
                    ...
                    if list(dict_e2.keys()) != month_list:
                        raise ValueError(
                            "All dictionary inputs must have an entry for every month!"
        ...
        if incorrect_type > 0 and incorrect_type != 6:
            raise TypeError(
                "You must either define all inputs as dictionaires"
    ...
    def _alter_if_leap_year(self, df_one_year, year):
        ...
        if len(df_one_year) == hour_in_year and feb_28.is_leap_year:
            ...
            df_feb28.index = pd.date_range(
                pd.Timestamp(year, 1, 1), periods=feb_28_day * hour_in_day, freq="H"
            )
            df_list.append(df_feb28)

            # repeat february 28th as a proxy for February 29th
            df_feb29 = df_one_year.iloc[
                feb_28_day * hour_in_day : (feb_28_day + 1) * hour_in_day, :
            ]
            df_feb29.index = pd.date_range(
                pd.Timestamp(year, 2, 29), periods=hour_in_day, freq="H"
            )
            df_list.append(df_feb29)

            # tag on the rest of the year.
            df_after_feb29 = df_one_year.iloc[
                feb_28_day * hour_in_day : (len(df_one_year) + 1) * hour_in_day
            ]
            df_after_feb29.index = pd.date_range(
                pd.Timestamp(year, 3, 1),
                periods=hour_in_leapyear - len(df_feb28) - len(df_feb29),
                freq="H",
            )
    ...
    def _process_wfile(
        ...
        new_wfile_name = create_output_weather_file_name(
        ...
        if (os.path.exists(new_wfile_path) and self._overwrite_existing) or (
            ...
            if new_input_format:
                ...
                for month, trans_matrix in transition_matrix.items():
                    ...
                    modified_trans_matrix = (
                        trans_matrix + transition_matrix_delta[month]
                    )

                    if not self.use_global:
                        coef, decay_func_type = self._coef_form(
                            max_avg_dist["param"][month], min_avg_dist["param"][month]
                        )
                    else:
                        coef = DEFAULT_NONE_DECAY_FUNC
                        decay_func_type = DEFAULT_NONE_DECAY_FUNC
                    ...
                    states_arr0 = objDM.history(
                        adj_num_step, state0, count_in_min_steps_intervals=False
                    )
                    ...
                    if test_markov:
                        ...
                        delt_between_hw = [
                            tup1[0] - tup0[0]
                            for tup1, tup0 in zip(
                                state_intervals[1][1:], state_intervals[1][0:-1]
                            )
                            if tup0[0] <= month_num_steps
                        ]
                        delt_in_hw = [
                            tup[1] - tup[0] + 1
                            for tup in state_intervals[1]
                            if tup[0] <= month_num_steps
                        ]
                        self._delTmax_verification_data["freq_s"].append(
                            {
                                "key_name": key_name,
                                "time delta between consecutive heat waves": delt_between_hw,
                                "month": month,
                ...
                objDM = DiscreteMarkov(
                    rng,
                    transition_matrix + transition_matrix_delta * (year - start_year),
                    state_name,
                    use_cython,
                )
            ...
            for (junk, state), s_ind in zip(state_intervals.items(), state_int):
                ...
                if s_ind == 1:
                    ...
                    for tup in state:
                        tup_try = (
                            int(tup[0] + min_steps / 2),
                            int(tup[1] + min_steps / 2),
                        )
                        if tup_try[0] >= len(states_arr) or tup_try[1] > len(
                            states_arr
                        ):  # go back
                            tup_try = (
                                int(tup[0] - min_steps / 2),
                ...
                for tup in state:
                    ...
                    new_vals, heat_added_0, delT_max_0, norm_temp, norm_dur = (
                        self._add_extreme(
                            org_series.iloc[tup[0] : tup[1] + 1],
                            avg_dist,
                            avg_delta,
            ...
            if write_results:
                ...
                if simple_call:
                    objA.write(new_wfile_path, overwrite=True, create_dir=True)
                else:
                    objA.write(
                        new_wfile_path,
                        overwrite=True,
    ...
    def _add_to_ground_temperatures(self, air_temp_change, alter_obj):
        ...
        if "epwobj" in dir(alter_obj):
            ...
            capitalized_headers = {
                header.upper(): header for header in alter_obj.epwobj.headers
            }
            if "GROUND TEMPERATURES" in capitalized_headers:
                ...
                new_ground_temps = self._ground_temp_data_dictionary_unfold(
                    ground_temps, air_temp_change
                )

                alter_obj.epwobj.headers[capitalized_headers["GROUND TEMPERATURES"]] = (
                    new_ground_temps
                )

                msg = (
                    "This epw file's dry bulb and ground temperatures have been altered "
                ...
                warn(
                    "Ground temperatures were not present in the epw headers and have not been changed!"
                )
        else:
            raise AttributeError(
                "The Extreme class object does not yet have an epwobj read in!"
            )
        pass
    ...
    def _ground_temp_data_dictionary_unfold(self, ground_temps, air_temp_change):
        ...
        for idx, gtemp in enumerate(ground_temps[1:]):
            ...
            if idx > 0 and idl > 3:
                new_ground_temps.append(
                    "{0:5.2f}".format(float(gtemp) + air_temp_change)
                )
            else:
    ...
    def total_energy_integral(A, B, D, D_odd, min_s, is_hw):
        ...
        return (
            2 * A * D_odd / np.pi
            + B * D
            - B * min_s / (2 * np.pi) * np.sin(2 * np.pi * D / min_s)
        )
    ...
    def _add_extreme(
        ...
        if shape_func_type is None or shape_func_type == "double_shape_func":
            ...
            if test_shape_func:
                ...
                h_test = Extremes.double_shape_func(
                    xval, Acoef, Bcoef, duration, min_steps
                )
                h_test_heat = np.trapezoid(h_test, xval)

                if (
                    100 * (h_test_heat - heat_added) / heat_added
                    > acceptable_error_percent
                ):
                    raise ExtremesIntegrationError(
                        "The heat wave increased "
                        + "temperature did not integrate to"
                        + " the heat_added. This should"
                        + " never happen. There is a bug."
                    )

            h_t = Extremes.double_shape_func(
                np.array(range(duration)), Acoef, Bcoef, duration, min_steps
            )
            # renormalize to make hourly approximation exactly fit heat_added
            h_t = heat_added * h_t / h_t.sum()

            new_vals = pd.Series(
                h_t, index=org_vals.index, name=org_vals.name, dtype=org_vals.dtype
            )
            ...
            s_month = org_dates.month.unique()[
                np.array(
                    [
                        (org_dates.month == month).sum()
                        for month in org_dates.month.unique()
                    ]
                ).argmax()
            ]
            ...
            if self.use_global:
                ...
                Sm1_E = (
                    _transform_fit_sup(
                        param["min energy per duration"],
                        param["hist max energy per duration"],
                        param["hist min energy per duration"],
                    )
                    + 1
                )
                Sm1_T = (
                    _transform_fit_sup(
                        param["min extreme temp per duration"],
                        param["hist max extreme temp per duration"],
                        param["hist min extreme temp per duration"],
                    )
                    + 1
                )
                S_E = (
                    _transform_fit_sup(
                        param["max energy per duration"],
                        param["hist max energy per duration"],
                        param["hist min energy per duration"],
                    )
                    - 1
                ...
                S_T = (
                    _transform_fit_sup(
                        param["max extreme temp per duration"],
                        param["hist max extreme temp per duration"],
                        param["hist min extreme temp per duration"],
                    )
                    - 1
            ...
            E_per_duration = integral_dist["func"](
                rng.random(1)[0], mu_E, sig_E, -1 + Sm1_E, 1 + S_E, minval_E, maxval_E
            )
            T_per_duration = integral_dist["func"](
                rng.random(1)[0], mu_T, sig_T, -1 + Sm1_T, 1 + S_T, minval_T, maxval_T
            )
            ...
            if self.use_global == False:
                if norm_temperature < 0.0:
                    ...
                    heat_added_0 = (
                        org_vals.values - norms_hourly["HLY-TEMP-NORMAL"].values
                    ).sum()
                    delT_max_0 = (
                        org_vals.values - norms_hourly["HLY-TEMP-NORMAL"].values
                    ).mean()
            ...
            # standard.
            heat_added = (
                E_per_duration * norm_energy * alpha_E * (duration / norm_duration)
                - heat_added_0
            )
            delTmax = (
                T_per_duration
                * norm_temperature
                * (alpha_T * (duration / norm_duration) + beta_T)
                - delT_max_0
            )
            ...
            if (is_hw and (delTmax > abs_maxval_delT)) or (
                not is_hw and (delTmax < abs_maxval_delT)
            ):
            ...
            Acoef = (delTmax - np.pi / (2 * D_odd) * heat_added) / (
                2
                - np.pi * D / (2 * D_odd)
                + (min_steps / (2 * D_odd)) * np.sin(2 * np.pi * D / min_steps)
            )
            ...
            if test_shape_func:
                ...
                h_test = Extremes.heat_wave_shape_func(
                    xval, Acoef, Bcoef, D, D_odd, min_steps
                )
            ...
            if iter0 >= max_iter:
                raise ValueError(
                    "The E and delTmax sampling could not find a physically real solution!"
                )

            if is_hw:
                self._delTmax_verification_data["delTmax"].append(
                    {
                        "delT_max_0": delT_max_0,
                        "delTmax": delTmax,
            ...
            h_t = Extremes.heat_wave_shape_func(
                np.array(range(duration)), Acoef, Bcoef, D, D_odd, min_steps
            )
            ...
            raise ValueError(
                "MEWS currently only can have 'double_shape_func' or 'heat_wave_shape_func'"
    ...
    def _coef_form(self, hw_param, cs_param):
        ...
        decay_func_type = {
            ABREV_WAVE_NAMES[0]: cs_param["decay function"],
