...
class Graphics:
    ...
    def plot_sample_dist_shift(
        ...
        fig, axl = plt.subplots(
            len(histT_tuple_dict),
            1,
            figsize=(10, 4 * len(histT_tuple_dict)),
            sharex=True,
            sharey=True,
        )
        ...
        for histT_tup, thresh_tup, ax in zip(
            histT_tuple_dict.items(), thresholds_dict.items(), axl
        ):
            labels_given = False
            bin_prev = None
            for wtype in events:
                ...
                if (bin0.sum() < 0 and bin_prev is None) or second_condition:
                    ...
                    if not binT is None:
                        ax.plot(
                            binT, histT[0], label=labels[1] + " " + wtype, color="red"
                        )
                ...
                if not ttup is None and is_temperature:
                    for tstr, ystr, pos, color, linestyle in zip(
                        ["actual", "target", "actual", "target"],
                        ["10 year", "10 year", "50 year", "50 year"],
                        [0, 0, 1, 1],
                        [
                            "blue",
                            mcd.CSS4_COLORS["darkblue"],
                            mcd.CSS4_COLORS["salmon"],
                            "red",
                        ],
                        [":", "--", ":", "--"],
                    ):
                        ...
                        if tstr == "target":
                            ...
                            hist_thresh = (
                                thresh - ipcc_shift[wtype]["temperature"][ystr]
                            )
                            ax.plot(
