...
class CMIP_Data(object):
    ...
    def _CMIP_Data_Collection(self, run_parallel):
        ...
        if self.data_folder is None:
            folder_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "..", "CMIP6_Data_Files"
            )
        else:
            folder_path = self.data_folder

        if not os.path.isabs(self.model_guide):
            self.model_guide = os.path.abspath(
                os.path.join(os.path.dirname(__file__), self.model_guide)
            )
            ...
            for row in range(2, model_guide_ws.max_row + 1):
                ...
                if not os.path.exists(path):
                    if run_parallel:
                        try:
                            t = thr.Thread(
                                target=self._Download_File, args=[path, link, semaphore]
                            )
                            t.start()
                            threads.append(t)
                        except:
                            logger.info(
                                "Parallel processing failed for threading! Reverting to non-parallel"
                            )
                            # start over
        ...
        for idx, scenario in enumerate(self.scenario_list):
            if run_parallel:
                total_model_list.append(
                    pool.apply_async(
                        self._Compiling_Data,
                        (
                            model_guide_wb,
                            folder_path,
                            scenario,
                ...
                self.total_model_data[scenario] = self._Compiling_Data(
                    model_guide_wb, folder_path, scenario
                )
        ...
        if run_parallel:
            ...
            for index in range(len(self.scenario_list)):
                self.total_model_data[total_model_list[index].get().scenario] = (
                    total_model_list[index].get()
                )
        ...
        self.delT = {
            scenario: self.total_model_data[scenario].delT_list_reg[
                self.end_year - self.baseline_year
            ]
            for scenario in self.scenario_list
    ...
    def _World_map_plotting(self):
        ...
        if self.use_cartopy and self.display_plots:
            ...
            ax.stock_img()
            plt.plot(
                [self.lon_desired, self.lon_desired],
            ...
            logger.warning(
                "Cannot display world map because cartopy is not properly installed"
            )
        ...
        def loc_download_func(path, link):
            if os.path.exists(path) == False:
                ...
                try:
                    ...
                    logger.warning(
                        "Download failed. Data node is down. Trying again with other nodes"
                    )
                    all_data_nodes = [
                        "aims3.llnl.gov",
                        "esgf-data.ucar.edu",
    ...
    def _Compiling_Data(self, model_guide_wb, folder_path, scenario):
            ...
            if not model_name in self.gcm_to_skip:
                ...
                if file_count == file_num:
                    if data_list != []:
                        ...
                        setattr(
                            scenario_model_data[Model.name],
                            "data",
        ...
        Total_obj = self.Total_data(
            scenario,
            self.baseline_year,
            self.end_year,
            self._cmip6_start_year,
            self._historical_scen_str,
        )
    ...
    class Total_data:
        ...
        def __init__(self, scenario, baseline_year, endyear, cmip_base_year, hist_str):
            if scenario != hist_str:
                ...
                self.year_temp_dict = {
                    new_list: [] for new_list in range(baseline_year, endyear + 1, 1)
                }
            else:
                ...
                self.year_temp_dict = {
                    new_list: []
                    for new_list in range(cmip_base_year, baseline_year + 1, 1)
                }
    ...
    def _yearly_dataset_temps(self, model_object, total_object):
        ...
        for year_index in range(end_year - start_year + 1):
            ...
            for month in range(1, 13):
                data = data_tas.isel(
                    time=int((years[year_index] - start_year) * 12 + month - 1)
                )
                ...
                if month == 1 and year_index == 0:
                    ...
                    if (
                        self.lat_desired <= lat_data_coords[0]
                        or self.lat_desired >= lat_data_coords[-1]
                    ):
                    ...
                    delta_lat = (lat_data_coords[-1] - lat_init) / (
                        len(lat_data_coords) - 1
                    )
                ...
                if (
                    x == -1 and self.lon_desired >= 300
                ):  # For data points that are in between [-1] and [0] on the left of lon = 0
                    diff_lon = lon_grid_coords_2 - (lon_grid_coords_1 - 360)
                    grid_lon_mult = abs(self.lon_desired - lon_grid_coords_1) / diff_lon
                elif (
                    x == -1
                ):  # For data points that are in between [-1] and [0] on the right of lon = 0
                    diff_lon = lon_grid_coords_2 - (lon_grid_coords_1 - 360)
                    grid_lon_mult = (
                        1 - abs(lon_grid_coords_2 - self.lon_desired) / diff_lon
                    )
                else:
                    grid_lon_mult = (
                        abs(self.lon_desired - lon_grid_coords_1) / delta_lon
                    )
                ...
                if grid_lat_mult > 1:
                    ...
                    lat_grid_coords_1 = lat_data_coords[y]
                    grid_lat_mult = (
                        abs(self.lat_desired - lat_grid_coords_1) / delta_lat
                    )

                # Location Error
                if month == 1 and year_index == 0 and self.calculate_error:

                    dist = np.sqrt(0.5) - np.sqrt(
                        (0.5 - grid_lon_mult) ** 2 + (0.5 - grid_lat_mult) ** 2
                    )
                    dist_error = (1 / np.sqrt(0.5)) * dist

                    # Normalization Error Boundaries.Set for the US
                    US_1_lon, US_1_lat = 235, 25
                    US_1_x, US_1_y = math.floor(
                        (US_1_lon - lon_init) / delta_lon
                    ), math.floor((US_1_lat - lat_init) / delta_lat)

                    US_4_lon, US_4_lat = 295, 50
                    US_4_x, US_4_y = math.ceil(
                        (US_4_lon - lon_init) / delta_lon
                    ), math.ceil((US_4_lat - lat_init) / delta_lat)
                ...
                # Normalization Error
                if self.calculate_error:
                    US_temp_array = data.values[
                        US_1_y:US_4_y, US_1_x:US_4_x
                    ]  # [rows,cols] -> [y,x]
                ...
                temp_des = (
                    temp_inter_1 * (1 - grid_lat_mult) + temp_inter_2 * grid_lat_mult
                )
            ...
            if (
                years[0] == self._cmip6_start_year
                and years[year_index] > self.baseline_year
            ):
                continue
    ...
    def _regress_scenario(
        self, scenario, years, sigma0, func, p0, baseline, delT_at_baseline
    ):
        """
        ...
        y_abs_data_1d = np.concatenate(
            [self.total_model_data[scenario].year_temp_dict[year] for year in years]
        )
        num_points = [
            len(self.total_model_data[scenario].year_temp_dict[year]) for year in years
        ]
        ...
        x_data_1d = np.concatenate(
            [np.ones(num_point) * year for num_point, year in zip(num_points, years)]
        )
        sigma = np.concatenate(
            [
                np.ones(num_point) * sigma0[year - years[0]]
                for num_point, year in zip(num_points, years)
            ]
        )
        ...
        regr_1d = np.concatenate(
            [
                np.ones(num_point) * regression[year - years[0]]
                for num_point, year in zip(num_points, years)
            ]
        )
        ...
        self.total_model_data[scenario].y_data_1d = (
            y_zero_data_1d  # arranged for scatter plots
        )
        self.total_model_data[scenario].x_data_1d = (
            x_data_1d  # arranged for scatter plots of the entire dataset.
        )
        ...
        for year_index in range(len(years)):
            dataset = (
                self.total_model_data[scenario].year_temp_dict[years[year_index]]
                - baseline
            )
            mean_dataset = np.mean(dataset)
            delT = dataset + delT_at_baseline
            lower_proj, upper_proj = st.t.interval(
                confidence=0.95,
                df=len(dataset) - 1,
                loc=np.mean(dataset),
    ...
    def _calculate_stats(self):
        ...
        """
        historical_years = [
            year for year in range(self._cmip6_start_year, baseline_year + 1, 1)
        ]
        ...
        baseline_year_avg_temp = np.mean(
            self.total_model_data[self._historical_scen_str].year_temp_dict[
                baseline_year
            ]
        )
        # baseline-1 average
        baseline_year_m1_avg_temp = np.mean(
            self.total_model_data[self._historical_scen_str].year_temp_dict[
                baseline_year - 1
            ]
        )
        ...
        preindustrial_baseline_temp = np.mean(
            np.concatenate(
                [
                    self.total_model_data[self._historical_scen_str].year_temp_dict[
                        year
                    ...
                    for year in np.arange(
                        self._cmip6_start_year, self._cmip6_baseline_end_year + 1
                    )
                ]
            )
        )
        ...
        for scenario in self.scenario_list:
            ...
            if scenario == self._historical_scen_str:
                ...
                if self._align_gcm_to_historical:
                    ...
                    for key, val in self.total_model_data[
                        scenario
                    ].year_temp_dict.items():
                        self.total_model_data[scenario].year_temp_dict[key] = (
                            val + offset_fact
                        )
    ...
    def results1(
        ...
        if self.display_plots:
            historical_years = [
                year
                for year in range(self._cmip6_start_year, self.baseline_year + 1, 1)
            ]
            future_years = [
                year for year in range(self.baseline_year, self.end_year + 1, 1)
            ]
            ...
            year_dict = {
                self._historical_scen_str: historical_years,
                "SSP119": future_years,
                "SSP126": future_years,
                "SSP245": future_years,
                "SSP370": future_years,
            ...
            scatter_label_dict = {
                self._historical_scen_str: "Averaged Historical Data",
                "SSP119": "Averaged SSP1-1.9 Data",
                "SSP126": "Averaged SSP1-2.6 Data",
                "SSP245": "Averaged SSP2-4.5 Data",
                "SSP370": "Averaged SSP3.70 Data",
                "SSP585": "Averaged SSP5-8.5 Data",
            }
            regression_label_dict = {
                self._historical_scen_str: "Historical Regression",
                "SSP119": "SSP1-1.9 Regression",
                "SSP126": "SSP1-2.6 Regression",
                "SSP245": "SSP2-4.5 Regression",
                "SSP370": "SSP3.70 Regression",
                "SSP585": "SSP5-8.5 Regression",
            }
            CI_label_dict = {
                self._historical_scen_str: "Historical Data 95% CI",
                "SSP119": "SSP1-1.9 Data 95% CI",
                "SSP126": "SSP1-2.6 Data 95% CI",
                "SSP245": "SSP2-4.5 Data 95% CI",
                "SSP370": "SSP3.70 Data 95% CI",
                "SSP585": "SSP5-8.5 Data 95% CI",
            }
                ...
                [
                    flat_df_list.append(
                        [
                            yr,
                            val,
                            "1/2 symmetric 95% confidence interval (CI) (add and subtract this from average to get CI)",
                            scenario,
                        ]
                    )
                    for yr, val in zip(
                        year_dict[scenario], self.total_model_data[scenario].CI_list
                    )
                ]

                [
                    flat_df_list.append(
                        [yr, val, "average of gcm data points", scenario]
                    )
                    for yr, val in zip(
                        year_dict[scenario], self.total_model_data[scenario].avg_list
                    )
                ]

            df = pd.DataFrame(
                flat_df_list,
                columns=["Year", "Change in Temperature (°C)", "Type", "Scenario"],
            )
            ...
            ax.set_position(
                [plot_box.x0, plot_box.y0, plot_box.width * 0.8, plot_box.height]
            )
            ax.legend(loc="center left", bbox_to_anchor=(1, 0.5))
            plt.xlabel("Year", fontdict={"fontsize": 26})
            plt.ylabel(
                "Change in Temperature from {0:d} (°C)".format(self.baseline_year),
                fontdict={"fontsize": 26},
            )
            plt.title(
                "Surface Temperature Change at lat=({0:.4f}, lon={1:.4f})".format(
                    self.lat_desired, self.lon_desired, self.baseline_year
                ),
                y=1.02,
                x=0.7,
    ...
    def results2(self, desired_scenario, resolution="low"):
        ...
        baseline = xr.DataArray(
            data=result_arrays[0], dims=["lat", "lon"], coords=[lat_array, lon_array]
        )
        projection = xr.DataArray(
            data=result_arrays[1], dims=["lat", "lon"], coords=[lat_array, lon_array]
        )
        ...
        if self.display_plots:
            if self.use_cartopy:
                ...
                delta_temp.plot.pcolormesh(
                    ax=ax, cmap="coolwarm", vmin=-limit, vmax=limit
                )
                ...
                plt.title(
                    f"US Temperature Change from {self.baseline_year} to {self.end_year} According to {desired_scenario}",
                    x=0.55,
                    fontdict={"fontsize": 16},
                )
            else:
                logger.warning(
                    "Cannot display US borders because cartopy is not properly installed"
                )
                fig = plt.figure()
                ax = plt.axes([0, 0, 2, 1])
                c = ax.pcolormesh(
    ...
    def _results2_calc(
        self, model_guide_ws, folder_path, scenario, model, lat_array, lon_array, year
    ):
        """
            ...
            if model_name == model:
                ...
                if file_count == file_num:
                    if data_list != []:
                        ...
                        data_list.append(Model.data)
                        setattr(
                            Complete_model, "data", xr.concat(data_list, dim="time")
                        )
                    else:
                        Complete_model = Model
                else:
        ...
        for i in range(len(lat_array)):
            for j in range(len(lon_array)):
                ...
                mon_temp = [0] * 12
                for month in range(1, 13):
                    data = Complete_model.data.isel(
                        time=int((year - Complete_model.start_year) * 12 + month - 1)
                    )
                    lon_data_coords = data.coords["lon"].values
                    lat_data_coords = data.coords["lat"].values
                    ...
                    temp_inter_2 = temp_3 * (1 - grid_lon_mult) + temp_4 * grid_lon_mult
                    temp_des = (
                        temp_inter_1 * (1 - grid_lat_mult)
                        + temp_inter_2 * grid_lat_mult
                    )
