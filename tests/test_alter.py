...
class Test_Alter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        erase_me_file = os.path.join(
            cls.test_weather_path, "erase_me_file", "erase_me_Altered.epw"
        )
    ...
    def test_shift_function(self):
        ...
        for constant_fmin in [True, False]:
            ...
            self.assertTrue(
                (db_before.mean() + 1.0 - 0.000001 < db_after.mean())
                and (db_before.mean() + 1.0 + 0.000001 > db_after.mean())
            )
            ...
            self.assertTrue(
                (db_before_extreme + 0.000001 > db_after_extreme)
            ...
            self.assertTrue(
                (db_before.mean() - 1.0 - 0.000001 < db_after2.mean())
                and (db_before.mean() - 1.0 + 0.000001 > db_after2.mean())
            )

            self.assertTrue(
                (db_before_extreme + 0.000001 > db_after2_extreme)
    ...
    def test_leapyear(self):
        ...
        wg_obj_test_leap_snip = Alter(
            os.path.join(
                self.test_weather_path, "URY_Montevideo.865800_IWEC_Feb28_snippet.epw"
            ),
            2024,
            True,
        )
        ...
        twenty_four_hours = len(wg_obj_test_leap_snip.epwobj.dataframe) - len(
            wg_obj_test_snip.epwobj.dataframe
        )
        self.assertTrue(twenty_four_hours == 24)
        num_line = 50154 - 8
        wg_long = Alter(
            os.path.join(self.test_weather_path, "Combined5YearWeather.epw"), 2020
        )

        wg_long.add_alteration(
            2023,
            10,
            1,
            3,
    ...
    def _Alter_config_use_exe(self, use_exe, binfilename, year, DST):
        ...
        if use_exe:
            ...
            obj_doe2 = Alter(binfilename, year, True, isdoe2=True, use_exe=use_exe)
    ...
    def test_doe2(self):
        ...
        binfilename = os.path.join(
            self.test_weather_path, "NM_Albuquerque_Intl_ArptTMY3.bin"
        )
        years = [2020, 2017]

        DSTs = [
            [
                datetime(years[0], 3, 8, 2, 0, 0, 0),
                datetime(years[0], 11, 1, 2, 0, 0, 0),
            ],
            [
                datetime(years[1], 3, 12, 2, 0, 0, 0),
                datetime(years[1], 11, 5, 2, 0, 0, 0),
            ],
        ]

        exe_path = os.path.join(os.path.dirname(__file__), "../third_party_software")
        exe_exist = os.path.exists(
            os.path.join(exe_path, "BIN2TXT.EXE")
        ) and os.path.exists(os.path.join(exe_path, "TXT2BIN.EXE"))
            ...
            for year, DST in zip(years, DSTs):
                if use_exe:
                    ...
                    if exe_exist:
                        ...
                        testDoe2 = False
                        warnings.warn(
                            "The DOE-2 features of MEWS cannot be tested"
                ...
                if testDoe2:
                    ...
                    obj_doe2 = self._Alter_config_use_exe(
                        use_exe, binfilename, year, DST
                    )

                    df_pre, df = self._add_cold_hot_and_climate(
                        obj_doe2, "DRY BULB TEMP (DEG F)", year
                    )
                    ...
                    if self.plot_results:
                        ...
                        ax.grid("on")
                        ax.set_title(
                            "Test DOE2 BIN Albuquerque, NM TMY3 with extreme sinusoidal heat wave, \ncold snap and exponential climate trend added"
                        )
                    ...
                    try:
                        obj_doe2_2 = self._Alter_config_use_exe(
                            use_exe, "erase_me.bin", year, DST
                        )
                    except Exception as e:
                        warnings.warn(
                            "The erase_me.bin file we just wrote cannot"
    ...
    def test_add_remove(self):
        ...
        df_pre, df = self._add_cold_hot_and_climate(
            wg_obj, "Dry Bulb Temperature", 2019
        )
        # df_pre = wg_obj.reindex_2_datetime()
        ...
        if self.plot_results:
            ...
            ax.grid("on")
            ax.set_title(
                "Test Santa Fe, NM TMY3 with extreme sinusoidal heat wave, \ncold snap and exponential climate trend added"
            )
        ...
        should_be_near_zero = (
            (
                wg_obj.reindex_2_datetime()["Dry Bulb Temperature"]
                - df_pre["Dry Bulb Temperature"]
            )
            .sum()
            .sum()
        )
        ...
        wg_obj.add_alteration(
            2019, start_day, start_month, start_hour, 5, delta_T, [1, 2, 3, 4, 5]
        )
    ...
    def test_Exceptions(self):
        ...
        with self.assertRaises(EPWMissingDataFromFile):
            file_path = os.path.join(
                self.test_weather_path, "ShorterThanAYearWeather.epw_messed_up"
            )
        ...
        with self.assertRaises(EPWFileReadFailure):
            file_path = os.path.join(
                self.test_weather_path,
                "DOE2_Must_Fail_LivermoreCA_HeatWave_Scenario2.bin",
            )
            wg_obj = Alter(file_path, 2021, False)
        ...
        with self.assertRaises(ValueError):
            wg_obj.add_alteration(
                2020, 1, 1, 1, 100, 10, alteration_name="DontRepeatNames!"
            )
            wg_obj.add_alteration(
                2020, 1, 1, 1, 100, 10, alteration_name="DontRepeatNames!"
            )
    ...
    def test_read_write(self):
        ...
        obj.read(
            os.path.join(self.test_weather_path, "URY_Montevideo.865800_IWEC.epw"), 2021
        )
        ...
        obj.write(
            os.path.join(self.test_weather_path, "erase_me.epw"),
            overwrite=True,
            create_dir=False,
        )
        ...
        erase_me_file = os.path.join(
            self.test_weather_path, "erase_me_file", "erase_me_Altered.epw"
        )
        obj.write(erase_me_file, False, True)
        os.remove(erase_me_file)
    ...
    def test_future_ability_to_translate_epw_to_doe2(self):
        ...
        translation_map = {
            ...
            "DAY OF MONTH": "Day",
            "HOUR OF DAY": "Hour",
            "WET BULB TEMP (DEG F)": "Dew Point Temperature",
