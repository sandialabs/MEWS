...
class Test_Extreme(unittest.TestCase):
    ...
    def test_extreme_exceptions(self):
        if self.run_tests:
            ...
            with self.assertRaises(TypeError):
                obj = DiscreteMarkov(
                    rng, self.tran_mat, ["A", "B", "C", "D"], "Not True"
                )
    ...
    def test_extreme_heat_history_EP_and_Doe2(self):
        ...
        if self.run_tests:
            ...
            for ido, wfiles, column in zip(isdoe2, fname, col):
                ...
                trans_mat = np.array(
                    [[1.0 - 2 * p_ee, p_ee, p_ee], [0.1, 0.9, 0.0], [0.1, 0.0, 0.9]]
                )
                # this matrices columns must add to zero
                trans_mat_delta = np.array(
                    [
                        [p_ee / 10, -p_ee / 20, -p_ee / 20],
                ...
                num_repeat = (
                    3  # number of times the weather file is repeated to form a history.
                )
                ...
                if ido:
                    doe_in = {
                        ...
                        "doe2_dst": [
                            datetime(year, 3, 14, 2, 0, 0, 0),
                            datetime(year, 11, 7, 2, 0, 0, 0),
                        ],
                ...
                if self.plot_results:
                    fig, axl = plt.subplots(
                        7, 1, sharex=True, sharey=True, figsize=(10, 10)
                    )

                    for realization_number, ax in enumerate(axl):

                        Graphics.plot_realization(
                            res,
                            column,
                            realization_number,
                            ax=ax,
    ...
    def test_ground_temperature_changes(self):
        if self.run_tests:
            ...
            # (i.e. 2 years into the future)
            num_repeat = (
                1  # number of times the weather file is repeated to form a history.
            )
