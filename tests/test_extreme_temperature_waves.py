...
class Test_ExtremeTemperatureWaves(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_weather_file_path = os.path.join(
            ".", cls.test_weather_path, "USA_NM_Santa.Fe.County.Muni.AP.723656_TMY3.epw"
        )
    ...
    def tearDownClass(cls):
        ...
        for file_name in os.listdir():
            if (
                "NM_Albuquerque_Intl_ArptTMY3" in file_name
                or "USA_NM_Santa.Fe.County.Muni.AP.723656" in file_name
                or "WEATHER.FMT" in file_name
                or "TXT2BIN.EXE" in file_name
                or "INPUT.DAT" in file_name
                or "USA_NM_Alb" in file_name
                or ".epw" in file_name
            ):
                try:
                    os.remove(file_name)
                    ...
                    warnings.warn(
                        "The testing could not clean up files"
        ...
        if os.path.exists(os.path.join(file_dir, "temp_out")):
            rmtree(
                os.path.join(file_dir, "temp_out"), ignore_errors=False, onerror=None
            )
        if os.path.exists(os.path.join(file_dir, "mews_results")):
            rmtree(
                os.path.join(file_dir, "mews_results"),
                ignore_errors=False,
                onerror=None,
            )
        ...
        if run_test:
            ...
            station = {
                "summaries": os.path.join(
                    self.file_dir, "data_for_testing", "example_with_repeated_date.csv"
                ),
                "norms": os.path.join(
                    self.file_dir, "data_for_testing", "repeated_date_norms.csv"
                ),
            }
    ...
    def test_albuquerque_extreme_waves(self):
        ...
        if run_test:
            ...
            weather_files = [
                os.path.join(
                    self.test_weather_path, "USA_NM_Albuquerque.Intl.AP.723650_TMY3.epw"
                )
            ]
    ...
    def test_ipcc_increases_in_temperature_and_frequency(self):
        ...
        if run_test:
            ...
            for start_year in start_years:
                ...
                results = obj.create_scenario(
                    scenario, start_year, climate_temp_func, num_realization=10
                )
            ...
            avg_delT_increase_hw = (
                np.array(scen_results[2050][0]) - np.array(scen_results[2020][0])
            ).mean()
            avg_delT_decrease_cs = (
                np.array(scen_results[2050][1]) - np.array(scen_results[2020][1])
            ).mean()

            freq_increase_hw = (
                np.array(scen_results[2050][2]) / np.array(scen_results[2020][2])
            ).mean()
            freq_increase_cs = (
                np.array(scen_results[2050][3]) / np.array(scen_results[2020][3])
            ).mean()

            expected_delT = climate_temp_func[scenario](2050) - climate_temp_func[
                scenario
            ](2020)
            ...
            self.assertTrue(
                ...
                and (avg_delT_increase_hw <= expected_delT + 0.2)
            )
            ...
            # documentation.)
            self.assertTrue(
                (freq_increase_hw <= 2.45 + 0.45) and (freq_increase_hw >= 2.45 - 0.45)
            )

            # cold snap frequency should be close to 1.0
            self.assertTrue(
                (freq_increase_cs <= 1.0 + 0.1) and (freq_increase_cs >= 1.0 - 0.1)
            )
        ...
        if run_test:
            ...
            solve_options = {
                ...
                "future": {
                    ...
                    "test_mode": True,
                    "min_num_waves": 10,
                    "out_path": os.path.join("temp_out", "future_test_output.png"),
                },
            }  # extra_output_columns must be altered later.
            ...
            metric_dict, meanval, obj, clim_scen, scen_dict = (
                self._run_verification_study_of_use_global_False(
                    num_realizations,
                    future_year,
                    random_seed,
                    self.plot_results,
    ...
    @staticmethod
    def _run_verification_study_of_use_global_False(
        num_realizations,
        future_year,
        random_seed,
        plot_results,
        ...
        for ci_int in ci_interval:
            ...
            res_dict[id0] = obj.create_scenario(
                scenario,
                2014,
                scen_dict,
            ...
            res_dict[ci_int] = obj.create_scenario(
                scenario,
                future_year,
                scen_dict,
                num_realization=num_realizations,
                ...
                for key3, objA in res.items():
                    ...
                    for month in range(1, 13):
                        num_hr_in_hw_d[key][month] += (
                            (df_status["Status"] == 1) & (df_status["month"] == month)
                        ).sum()
                    ...
                    for key4, alt in objA.alterations.items():
                        if key4 != "global temperature trend":
                            for month in range(1, 13):
                                ...
                                num_in_month = (
                                    Test_ExtremeTemperatureWaves().get_hw_hour_count(
                                        df_status, alt, month
                                    )
                                )
                                ...
                                if num_in_month == 0:
                                    ...
                                    num_in_next_month = Test_ExtremeTemperatureWaves().get_hw_hour_count(
                                        df_status, alt, next_month
                                    )
                    ...
                    check_hr_count = np.array(
                        [num_hr_in_hw_d[key][mon] for mon in range(1, 13)]
                    ).sum()
    ...
    def test_read_and_write_solution_file(self):
        ...
        if run_test:
            ...
            solve_options = {
                "historic": {"num_step": 20000, "test_mode": True},
                "future": {"num_step": 20000, "test_mode": True},
            }

            # worcester, MA climate function solution.
            scen_dict = {
                "historical": np.poly1d(
                    [
                        1.35889778e-10,
                        7.56034740e-08,
                        1.55701410e-05,
            ...
            new_stats = obj.read_solution(
                os.path.join("temp_out", "test_solution_file.txt")
            )

            obj2 = ExtremeTemperatureWaves(
                station,
                weather_files,
                unit_conversion=(1 / 10, 0),
                use_local=True,
    ...
    def test_create_solutions(self):
        ...
        if run_test:
            ...
            historical_solution = os.path.join(
                inp_dir, "worcester_historical_solution.txt"
            )
            ...
            weather_files = [
                os.path.join(inp_dir, "USA_MA_Worcester.Rgnl.AP.725095_TMY3.epw")
            ]
            ...
            self.assertTrue(
                os.path.exists(
                    os.path.join(os.path.dirname(historical_solution), filenames[0])
                )
            )
            try:
                os.remove(
            ...
            warnings.warn(
                "The test_create_solutions unittest was not run because it is turned off!"
            )
