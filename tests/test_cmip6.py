...
class TestCMIP(unittest.TestCase):
    ...
    def test_temperature_data_collection(self):
        ...
        self.assertFalse(
            np.array_equal(np.array([]), test_obj.total_model_data["SSP585"].delT_list)
        )
        self.assertFalse(
            np.array_equal(
                np.array([]), test_obj.total_model_data["SSP585"].delT_list_reg
            )
        )
        self.assertFalse(
            np.array_equal(np.array([]), test_obj.total_model_data["SSP585"].CI_list)
        )
