...
def thread_function(realization, name, base_name, base_folder, files):
    ...
    if realization < 10:
        ...
        raise ValueError(
            "The script only allows realizations up to 999 expand the code to include higher cases"
        )
    ...
    with ZipFile("Kodiak" + str(realization) + ".zip", "w", ZIP_DEFLATED) as zippy:
        for file in files:
            if (
                (base_name in file)
                and str(realization) in file[end_ind:]
                and file[end_ind - 1] == "r"
            ):
                zippy.write(file)
                print(
                    "Count={0:d},Realization={1:d},File={2}".format(
                        count, realization, file
                    )
                )
                count += 1
...
base_folder = os.path.join(
    os.path.dirname(__file__), "example_data", "Worcester", "mews_results_krissy2"
)
...
for realization in range(100):
    x = threading.Thread(
        target=thread_function, args=(realization, name, base_name, base_folder, files)
    )
    threads.append(x)
    x.start()
