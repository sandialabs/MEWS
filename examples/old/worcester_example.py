...
if __name__ == "__main__":
    ...
    first_try_solution_location = os.path.join(
        "example_data", "Worcester", "results2", "worcester_historical_solution.txt"
    )
    ...
    if step == 1:
        ...
        pkl.dump(
            [cs_obj],
            open(os.path.join("temp_pickles", "cs_obj_worcester.pickle"), "wb"),
        )
        ...
        cs_obj = pkl.load(
            open(os.path.join("temp_pickles", "cs_obj_worcester.pickle"), "rb")
        )[0]
    """
    ...
    if step == 2:
        solve_options = {
            "historic": {
                ...
                "out_path": os.path.join(
                    "example_data", "Worcester", "results2", "worcester.png"
                ),
            },
            "future": {
                "delT_above_shifted_extreme": {"cs": -10, "hw": 10},
                ...
                "min_num_waves": 10,
                "out_path": os.path.join(
                    "example_data", "Worcester", "results2", "worcester_future.png"
                ),
            },
        }
        ...
        pkl.dump(
            [obj, solve_options],
            open(os.path.join("temp_pickles", "obj_worcester.pickle"), "wb"),
        )
    else:

        blob = pkl.load(
            open(os.path.join("temp_pickles", "obj_worcester.pickle"), "rb")
        )
        obj = blob[0]
