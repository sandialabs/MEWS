...
from mews.weather import ClimateScenario
...
class Input:
    ...
    station = {
        "summaries": os.path.join(
            "example_data", "ClimateZone4B_Albuquerque", "USW00023050.csv"
        ),
        "norms": os.path.join(
            "example_data", "ClimateZone4B_Albuquerque", "USW00023050_norms.csv"
        ),
    }
    weather_file_name = os.path.join(
        "ClimateZone4B_Albuquerque", "USA_NM_Albuquerque.Intl.AP.723650_TMY3.epw"
    )
    ...
    path_idf_files = [
        os.path.join(
            main_path, "example_data", "ASHRAE901_OfficeMedium_STD2019_Albuquerque.idf"
        )
    ]
...
def post_process_single_run(epw, idf):
    ...
    multi_index_names = ["Variable Name", "Scenario", "Realization", "Year"]
        ...
        for start_year in self.start_years:
            for scenario in self.scenarios:
                ...
                obj.create_scenario(
                    ...
                    self.num_year,
                    climate_temp_func,
                    num_realization=self.num_realizations,
                )
    ...
    if run_local:
        ...
        try:
            opts, args = getopt.getopt(
                sys.argv[1:], "s:y:r", ["scenario=", "start_year=", "random_seed="]
            )
        ...
        if not opts:
            print("")
            print(
                'No command line options given, you must have "--scenario=<IPCC scenario name>" and "--start_year=<year>"'
            )
            print("")
            sys.exit(2)
