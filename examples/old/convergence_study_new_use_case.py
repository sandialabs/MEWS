    ...
    for key, sub2dict in subdict.items():
        for key2, sub3dict in sub2dict.items():
            ...
            if arr is None:
                ...
                arr = np.concatenate(
                    [arr, temp_arr.mean(axis=0).reshape([1, temp_arr.shape[1]])], axis=1
                )
...
model_guide = os.path.join(
    fpath, "..", "mews", "data_requests", "data", "Models_Used_alpha.xlsx"
)
data_folder = os.path.join(fpath, "..", "..", "CMIP6_Data_Files")
station = os.path.join(
    fpath, "..", "mews", "tests", "data_for_testing", "USW00023050.csv"
)
weather_files = [
    os.path.join(
        fpath,
...
if runme:
    ...
    for ca, rs in zip(convergence_array, random_seed):
        ...
        if initiate_runs:
            start = time()
            metric_meta_dict[(ca, rs)], meanvals, objExtW, clim_scen, scen_dict = (
                obj._run_verification_study_of_use_global_False(
                    ca,
                    2080,
                    rs,
                    False,
            ...
            print(
                "\nThe {0} case took {1:5.2f} minutes to run.\n\n".format(
                    str((ca, rs)), (end - start) / seconds_in_minute
                )
            )
