...
class Input:
    ...
    station = {
        "summaries": os.path.join(
            "example_data", "ClimateZone5B_Denver", "Denver_daily.csv"
        ),
        "norms": os.path.join(
            "example_data", "ClimateZone5B_Denver", "Denver_norms.csv"
        ),
    }
    weather_file_name = "USA_NM_Albuquerque.Intl.AP.723650_TMY3.epw"
    weather_files = [
        os.path.join("example_data", "ClimateZone4B_Albuquerque", weather_file_name)
    ]
...
def run_and_post_process_ep(tup):
    ...
    if not only_post_process:
        ...
        run_succeeded = execute_command(
            [os.path.join(ep_path, "energyplus"), "-w", "in.epw", "in.idf"]
        )
    else:
