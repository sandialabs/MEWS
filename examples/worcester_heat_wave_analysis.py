    ...
    for year in years:
        for scenario in scenarios:
            for ci in confidence_intervals:
                weather_files.append(base_weather_file_name[:-4] + scenario + "_" + str(year) + "_" + ci)
...
def read_eplus_subtable(df,report_name,sub_report_name,scenario,realization,
                        year,ipcc_confidence_interval, multi_index_names,dtypes=None,to_dates=None,rows_wanted=None):
    ...
    tup_list = [(varname,scenario,realization,year,ipcc_confidence_interval) for varname in table.index]
...
def run_and_post_process_ep(tup):
    ...
    if not only_post_process and not os.path.exists("eplustbl.csv"):
        ...
        run_succeeded = execute_command([os.path.join(ep_path,exe_name), "-w", 'in.epw', 'in.idf'])
    else:
            ...
            for epw in list_EPWs_HW:
                for realization in range(Inp.num_realizations):
                    ...
                    if os.path.exists(os.path.join(path_mews_files_HW,epw_r)):
                        ...
                        warn("Weatherfile Path does not exist? This may be ok if "+
