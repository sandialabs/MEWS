...
record_identifiers = {
    ...
    "G": "Genesis",
    "T": {
        "help": "Provides additional detail on the track "
...
data = pd.read_csv(
    os.path.join(
        ...
        "weather",
        "hurricanes",
        "hurdat2-1851-100522_with_column_names_added.txt",
    )
)

pr_lat_lons = {
    "Aguadilla": [18.50, -67.13],
    "San Juan": [18.43, -65.99],
    "Penuelas": [17.97, -66.77],
}

roman_et_al_data = {"Aguadilla": 87, "San Juan": 111, "Penuelas": 68}
    ...
    for row_num, row in data.loc[start_row_1982:].iterrows():
        ...
        if row["date"][0] != "A":
            ...
            if "L" in row["record identifier"] or "C" in row["record identifier"]:
                ...
                temp_dist = latlon_distance(
                    latlon, [float(row[' "Latitude"'].split("N")[0]), lon]
                )
                if temp_dist < pr_radius:
                    dist_list.append(
                        [temp_dist, row["Max Wind Speed (knots)"], row["date"]]
                    )
        else:
            if is_first:
...
for key, dist in distance.items():
    ...
    fit = curve_fit(
        ...
        cdf,
        np.array([0.0, 100]),
        bounds=[(0.0, 10.0), (0.0000001, 1000)],
        full_output=True,
    )
    ...
    pass
    estimated_average_power_outage_duration[key] = roman_et_al_data[key] * (
        fit[0][1] / Maria_ws
    )
